...
<?php
   ...
   //select the last transaction id in the sender's passbook
    ...
    $date=date("Y-m-d");
	$r_total=$r_amount+$t_amount; //Receivers total
    $s_total=$s_amount-$t_amount; //senders total
    ...
    elseif($s_total<500)
    {
        ...
        $sql1="insert into passbook".$reciever_id." values('','$date','$r_name','$r_branch', '$r_acc_type','$r_total','0','$r_total','BY $s_name')";
        mysql_query($sql1) or die(mysql_error());
        
		//Withdraw from User account
        //insert statement into sender passbook.
        $sql2="insert into passbook".$sender_id." values('','$date','$s_name','$s_branch','$s_acc_type','0','$t_amount','$s_total','TO $r_name')";
